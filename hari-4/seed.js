...
async function seed() {
  try {
    ...
    for (const data of dataContoh) {
      aduanDicipta.push(await Aduan.create(data));
    }

    // Tindakan susulan contoh (hubungan: rujuk aduan pertama & kedua).
    await Tindakan.create({
      aduan: aduanDicipta[1].id,
      pegawai: 'Pegawai Penguatkuasa Selangor',
      catatan: 'Lawatan pemeriksaan ke premis dijalankan. Sampel timbangan diambil.',
      statusBaru: 'Dalam Siasatan',
    });
    await Tindakan.create({
      aduan: aduanDicipta[2].id,
      pegawai: 'Pegawai Penguatkuasa Johor',
      catatan: 'Kompaun dikeluarkan kepada premis. Kes ditutup.',
      statusBaru: 'Selesai',
    });

    console.log(`✅ ${dataContoh.length} aduan + 2 tindakan susulan berjaya dimasukkan.`);
  } catch (error) {
